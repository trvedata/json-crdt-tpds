...
Without loss of generality, assume that $o_r.\mathit{id} < o_s.\mathit{id}$ according to the ordering relation on Lamport timestamps. (If the operation ID of $o_r$ is greater than $o_s$, $o_r$ and $o_s$ can be swapped in this proof.) We now distinguish two cases:
...
\item $o_s$ is applied before $o_r$ in $H$. Thus, at the time when $o_r$ is applied, $o_s$ and any insertions causally prior to $o_s$ in the same insertion interval have already been applied.
\end{enumerate}

In both cases, the insertion position of $o_r$ appears after the insertion position of $o_s$. Since the argument applies to all pairs of concurrent operations $o_r, o_s$ in $H$, we deduce that the final order of elements in the list depends only on the operation IDs but not the order of application, which shows that concurrent insertions to the same list are commutative.
\end{proof}
