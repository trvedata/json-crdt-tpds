...
\node (start)  at (2,4) {\verb|{"key":"A"}|};
\node (left1)  at (0,1.5) {\verb|{"key":"B"}|};
\node (right1) at (4,1.5) {\verb|{"key":"C"}|};
\node (left2)  at (0,0) {\verb|{"key":{"B","C"}}|};
\node (right2) at (4,0) {\verb|{"key":{"B","C"}}|};
\draw [->] (start) to node [above,sloped] {\footnotesize $\text{doc}[\text{``key''}] := \text{``B'';}$} (left1);
\draw [->] (start) to node [above,sloped] {\footnotesize $\text{doc}[\text{``key''}] := \text{``C'';}$} (right1);
\draw [->] (left1) to node [left] {\footnotesize yield;} (left2);
\draw [->] (left1) to (right2);
\draw [->] (right1) to (left2);
\draw [->] (right1) to node [right] {\footnotesize yield;} (right2);
\end{tikzpicture}
