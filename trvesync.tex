...
& $|$ & $\mathrm{EXPR}.\mathsf{incr}(n)$ & $n \in \mathbb{N}$ \\
& $|$ & \textsf{yield} \\
...
The syntax of the command language is given in Figure~\ref{fig:local-syntax}. A program is assumed to be a linear, possibly infinite, sequence of commands. For brevity, the language of Figure~\ref{fig:local-syntax} does not provide the usual control structures such as conditionals, loops, or functions, but such facilities can easily be implemented by embedding this command language in a general-purpose programming language.

We first explain the language informally, before giving its formal semantics. The expression construct EXPR is used to query the state of a document. An expression starts with either the special token \textsf{doc}, identifying the root of the JSON document tree, or a variable $x$ that was previously defined in a \textsf{let} command. The expression then continues with a sequence of method calls: $\mathsf{get}(\mathit{key})$ selects a key within a map (a JSON object), \textsf{iter} starts iterating over an ordered list, \textsf{next} moves to the next element of an ordered list, and \textsf{values} returns the value at the current position within the document.

A command CMD either sets the value of a local variable (\textsf{let}), performs network communication (\textsf{yield}), or modifies the document. A document can be modified by assigning the value of some field (using the assignment operator :=), by inserting an element into a list (\textsf{insert}), by deleting an element from a list or a map (\textsf{delete}), or by incrementing a counter (\textsf{incr}). The preceding expression EXPR acts as a cursor, identifying the part of the document being modified.

Figure~\ref{fig:make-doc} shows an example of a sequence of commands constructing a new document representing a shopping list. First \textsf{doc} is set to \verb|{}|, the empty map literal. The second line navigates to the key \verb|"shopping"| (which is implicitly created if it does not exist), and calls \textsf{iter}, which treats the value at that key as a list and selects the head of the list. Finally, three items are inserted into the list. The \textsf{insert} command adds a new list element \emph{after} the current cursor position, and returns the cursor position of the newly inserted element. Thus, milk is inserted after eggs, but cheese is inserted at the head of the list, because the variable \verb|list| continues to refer to the head.

A few features of this language deliberately differ from most mainstream programming languages: keys in maps are implicitly created when they are first accessed; lists can only be navigated by iteration (\textsf{next}) but not by index; and the language has literals for creating empty maps and lists, but not for non-empty collections. As we shall see later, these features are helpful for achieving desirable semantics in the presence of concurrent modifications.

\subsubsection{Semantics of expression evaluation}

\begin{figure}
\centering \begin{alignat*}{2}
A_p =\; && \{\; \mathsf{mapT}(\mathsf{doc}) & \mapsto \{\; \mathsf{listT}(\text{``shopping''}) \mapsto \{ \\
&& \mathsf{next}(\mathsf{head}) & \mapsto \mathit{id}_3, \\
&& \mathsf{regT}(\mathit{id}_3) & \mapsto
    \{\,\mathsf{op}(\mathsf{id}: \mathit{id}_3,\, \mathsf{val}: \text{``cheese''})\,\}, \\
&& \mathsf{next}(\mathit{id}_3) & \mapsto \mathit{id_1}, \\
&& \mathsf{regT}(\mathit{id}_1) & \mapsto
    \{\,\mathsf{op}(\mathsf{id}: \mathit{id}_1,\, \mathsf{val}: \text{``eggs''})\,\}, \\
&& \mathsf{next}(\mathit{id}_1) & \mapsto \mathit{id_2}, \\
&& \mathsf{regT}(\mathit{id}_2) & \mapsto
    \{\,\mathsf{op}(\mathsf{id}: \mathit{id}_2,\, \mathsf{val}: \text{``milk''})\,\}, \\
&& \mathsf{next}(\mathit{id}_2) & \mapsto \mathsf{tail} \\
\}\;\}\;\}
\end{alignat*}
\caption{Internal representation of the example document from Figure~\ref{fig:make-doc}.}\label{fig:state-example}
\end{figure}
...
An illustrative example of the peer state $A_p$ is given in Figure~\ref{fig:state-example}, which corresponds to the shopping list example of Figure~\ref{fig:make-doc}. In Figure~\ref{fig:state-example}, $A_p$ is a partial function whose domain is the single atom \textsf{mapT(doc)}, which denotes that the document \textsf{doc} is of type map. \textsf{mapT(doc)} maps to another partial function containing the key-value pairs of the map; here, the only entry is the key ``shopping'' of type \textsf{listT}. The list is represented in a manner resembling a linked list, with each list element assigned a unique identifier ($\mathit{id}_1, \mathit{id}_2, \mathit{id}_3$), and special \textsf{head} and \textsf{tail} atoms denoting the beginning and end of the list, respectively.

Figure~\ref{fig:expr-rules} gives the rules for evaluating EXPR expressions in the command language, which are evaluated in the context of the local peer state $A_p$. The \textsc{Exec} rule defines that commands are executed sequentially. The \textsc{Let} rule allows the program to define a local variable, which is added to the local state, and the corresponding \textsc{Var} rule allows the program to retrieve the value of a previously defined variable.
...
Finally, the $\textsc{Val}_{1,2,3}$ rules allow a program to read the contents of the document at a particular cursor position, using a similar recursive rule structure as the \textsc{Next} rules. The leaves of the document tree are always \emph{registers}, expressed using the \textsf{regT} type annotation in the local state. Although a peer can only assign a single value to a register, a register can nevertheless contain multiple values if multiple peers concurrently assign values to it, an issue we will explore in greater depth later.

\subsubsection{Semantics of local mutations}
...
\AxiomC{$\mathit{queue} = A_p'(\mathsf{queue}) \,\cup\, \{(A_p'(\mathsf{ops}), \mathit{op})\}$}
\LeftLabel{\textsc{Apply-Local}}
\BinaryInfC{$A_p,\, \mathsf{apply}(\mathit{op}) \evalto A_p'[\,
    \mathsf{queue} \,\mapsto\, \mathit{queue},\;
    \mathsf{ops} \,\mapsto\, A_p''(\mathsf{ops}) \,\cup\, \{\mathit{op}\}\,]$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$(\mathit{deps}, \mathit{op}) \in A_p(\mathsf{recv})$}
\AxiomC{$\mathit{deps} \subseteq A_p(\mathsf{ops})$}
\AxiomC{$A_p,\, \mathit{op} \evalto A_p'$}
\LeftLabel{\textsc{Apply-Remote}}
\TrinaryInfC{$A_p,\, \mathsf{yield} \evalto
    A_p'[\,\mathsf{ops} \,\mapsto\, A_p'(\mathsf{ops}) \,\cup\, \{\mathit{op}\}\,]$}
\end{prooftree}
\caption{Rules for Lamport timestamps, sending, receiving, and applying operations}
\label{fig:send-recv}
\end{figure*}
...
\AxiomC{$A_p',\, \mathsf{apply}(\mathsf{listOp}(\mathit{id})) \evalto A_p''$}
\LeftLabel{\textsc{Alloc-List}}
\BinaryInfC{$A_p,\, \mathsf{listCmd} \evalto A_p'',\, \mathit{id}$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\mathit{prev} \in \mathrm{dom}(A_p)$}
\AxiomC{$A_p,\, \mathsf{newID} \evalto A_p',\, \mathit{id}$}
\AxiomC{$A_p',\, \mathsf{apply}(\mathsf{insertOp}(\mathit{id}, \mathit{prev}, v)) \evalto A_p''$}
\LeftLabel{\textsc{Insert}}
\TrinaryInfC{$A_p,\, \mathsf{insertCmd}(\mathit{prev}, v) \evalto A_p'',\, \mathit{id}$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\mathit{target} \in \mathrm{dom}(A_p)$}
\AxiomC{$A_p,\, \mathsf{newID} \evalto A_p',\, \mathit{id}$}
\AxiomC{$A_p',\, \mathsf{apply}(\mathsf{deleteOp}(\mathit{id}, \mathit{target})) \evalto A_p''$}
\LeftLabel{\textsc{Delete}}
\TrinaryInfC{$A_p,\, \mathsf{deleteCmd}(\mathit{target}) \evalto A_p'',\, \bot$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\mathit{id} \not\in \mathrm{dom}(A_p)$}
\AxiomC{$A_p,\, \mathsf{ts}(\mathit{id}) \evalto A_p'$}
\LeftLabel{\textsc{Apply-List}}
\BinaryInfC{$A_p,\, \mathsf{listOp}(\mathit{id}) \evalto
    A_p'[\,\mathit{id} \,\mapsto\, \mathsf{listEl}(\bot, \mathsf{tail})\,]$}
\end{prooftree}
...
\QuaternaryInfC{$A_p,\, \mathsf{insertOp}(\mathit{id}, \mathit{prev}, v) \evalto
