...
\subsection{Command language introduction}
...
\subsection{Local expression evaluation}
...
In order to generate globally unique operation identifiers without requiring synchronous coordination between peers we use Lamport timestamps~\cite{Lamport:1978jq}. A Lamport timestamp is a pair $(c, p)$ where $p \in \mathrm{PeerID}$ is the unique identifier of the peer on which the edit is made (for example, a cryptographic hash of its public key), and $c \in \mathbb{N}$ is a counter that is stored at each peer and incremented for every operation. Since each peer generates a strictly monotonically increasing sequence of counter values $c$, the pair $(c, p)$ is unique.
