...
\BinaryInfC{$\mathit{ctx},\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, k), \mathsf{assign}(\mathit{val})) \evalto
    \mathit{ctx}[\, \mathsf{regT}(k) \,\mapsto\, \{\, \mathit{id} \,\mapsto\, \mathit{val} \,\}\,]$}
\end{prooftree}
...
\AxiomC{$ctx,\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, \mathit{next}), \mathsf{assign}(\mathit{val})) \evalto \mathit{ctx}'$}
...
\begin{prooftree}
\AxiomC{$\mathsf{next}(\mathit{target}) \in \mathrm{dom}(\mathit{ctx})$}
\LeftLabel{\textsc{List-Del}}
\UnaryInfC{$\mathit{ctx},\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, \mathit{target}), \mathsf{listDel}) \evalto
    \mathit{ctx}[\,\mathsf{deleted}(\mathit{target}) \,\mapsto\, \mathsf{true}\,]$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\mathsf{regT}(k) \in \mathrm{dom}(\mathit{ctx})$}
\AxiomC{$\mathit{concurrent} = \{ \mathit{id}_i \mapsto v_i \mid
    (\mathit{id}_i \mapsto v_i) \in \mathit{ctx}(\mathsf{regT}(k))
    \,\wedge\, \mathit{id}_i \notin \mathit{deps} \}$}
\LeftLabel{\textsc{Clear-Reg}}
\BinaryInfC{$\mathit{ctx},\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, k), \mathsf{clearReg}) \evalto
    \mathit{ctx}[\, \mathsf{regT}(k) \,\mapsto\, \mathit{concurrent} \,]$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\mathsf{listT}(k) \in \mathrm{dom}(\mathit{ctx})$}
\AxiomC{$\mathit{ctx}(\mathsf{listT}(k)),\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, \mathsf{head}), \mathsf{clearElems}) \evalto \mathit{cleared}$}
...
\begin{prooftree}
\AxiomC{$k \not= \mathsf{tail} \,\wedge\, k \in \mathit{deps}$}
\AxiomC{$\mathit{ctx}(\mathsf{next}(k)) = \mathit{next}$}
\AxiomC{$\mathit{ctx},\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, \mathit{next}), \mathsf{clearElems}) \evalto \mathit{ctx}'$}
\LeftLabel{$\textsc{Clear-Elems}_1$}
\TrinaryInfC{$\mathit{ctx},\, \mathsf{op}(\mathit{id}, \mathit{deps},
    \mathsf{cursor}(\langle\rangle,\, k), \mathsf{clearElems}) \evalto
    \mathit{ctx}'[\,\mathsf{deleted}(k) \,\mapsto\, \mathsf{true}\,]$}
\end{prooftree}
...
\begin{enumerate}
\item Distinction between \textsf{listDel} and \textsf{mapDel}
\item Assignment of empty list and empty array is not defined (need to reset value, but preserve its existence and datatype) -- also needed for creating nested lists (2D arrays)
\item \textsf{.next} should skip over tombstones
\end{enumerate}
